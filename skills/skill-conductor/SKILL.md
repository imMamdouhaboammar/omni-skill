...
| 7. DISTRIBUTE | "craft repo", "distribute skill", "skills.sh"   | Scaffolds manifests (package.json, marketplace.json, .skills.json, install.sh), GitHub repo & CI |
...
Turn a verified skill into a high-presence GitHub repository distributed across the entire AI agent ecosystem (Claude Code, Claude Marketplace, Skills.sh, npm/npx, Cursor, Codex, OpenCode, Antigravity, Gemini CLI).

1. Scaffold distribution layer:

```bash
bun scripts/scaffold-repo.ts <skill-folder> [--name <name>] [--author <author>] [--desc <pitch>]
```

2. Review generated manifests:
- `package.json` with npm binary CLI runner
- `marketplace.json` conforming to Claude Plugin schema
- `.skills.json` for Skills.sh registry
- `install.sh` for multi-agent automated installation

3. Follow the full distribution canon → **`references/repo-craft-and-distribute.md`**.
...
| `references/repo-craft-and-distribute.md` | Repo crafting, multi-agent manifests, GitHub topics, & Skills.sh |
...
| `scripts/scaffold-repo.ts`       | Universal distribution scaffolder (Bun)    |
